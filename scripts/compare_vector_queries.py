...
def run_query(client: Redis, index_name: str, query: str, vector: bytes):
...
def main() -> int:
    ...
    aggregate = {
        ...
        "filtered": {ef: {"times": [], "recalls": []} for ef in ef_values},
    }
    ...
    for sample_index, player_id in enumerate(sample_player_ids, start=1):
        ...
        for ef_runtime in ef_values:
            ...
            matches, elapsed_ms, ids = run_query(client, args.index_name, unfiltered_query, vector)
            ...
            matches, elapsed_ms, ids = run_query(client, args.index_name, filtered_query, vector)
            aggregate["filtered"][ef_runtime]["times"].append(elapsed_ms)
            if reference_filtered_ids is None:
                reference_filtered_ids = ids
                recall = 1.0
            else:
                _, recall = overlap(reference_filtered_ids, ids)
            aggregate["filtered"][ef_runtime]["recalls"].append(recall)
            print(
                f"  filtered   ef={ef_runtime} matches={matches} time_ms={elapsed_ms:.2f} recall_vs_128={recall:.2%}"
            )
        print()
